...
export default class Input extends LitElement {
    ...
    static get styles() {
        return css`${unsafeCSS(style(require("!css-loader!sass-loader!styles/components/color-input.scss")))}`
    }

    render() {
        return html`
        <div class="colors-colorinput ${this.class_name}">
            <input id="color-value" class="colors-colorinput__input" @input=${this.updateColor} />
            <span id="color-preview" class="colors-colorinput__preview"></span>
            <button class="colors-colorinput__button" @click=${this.showColorInfo}>${this.label}</button>
        </div>
        `;
    }
    ...
    showColorInfo(e: Event) {
        let input = this.shadowRoot?.getElementById("color-value") as HTMLInputElement;
        if (input) {
