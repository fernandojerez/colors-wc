...
function produce_colors(color: Color, fn: (c: Color, n: number) => Color): string[] {
    let c = Color(color);
    const result = [c.hex()];
    for (let i = 1; i < 11; i++) {
...
const triadic = (color: Color) => [color.hex(), color.rotate(120).hex(), color.rotate(240).hex()];
const analogous = (color: Color) => [color.hex(), color.rotate(30).hex(), color.rotate(330).hex()];
...
export class Panel extends LitElement {
    ...
    render() {
        return html`
            <colors-tileset colors='${this._produceColors()}' mode="compact"></colors-tileset>
        `;
    }
